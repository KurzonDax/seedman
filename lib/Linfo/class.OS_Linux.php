...
class OS_Linux extends OS_Unix_Common {
    ...
    /**
     *  Get disk quota usage for user for non-dedicated seedboxes that use disk
     *  quotas
     */
    public function getQuota() {

        $checkCommand = shell_exec("quota");
        if (empty($checkCommand))
        {
            return false;
        }

        $quotaRaw=shell_exec("quota | grep dev | awk '{ print $2\",\"$3 }'");
        $quotaRawArray=explode(',',$quotaRaw);

        return array('used' => round(($quotaRawArray[0]/1048576),0), 'total' => round(($quotaRawArray[1]/1048576),0));
    }
